                ...
                <Box sx={{
                    ...
                    { sSrData?.map((post:any) : any  =>{
                        if(blogsId === post.tags.find(((tag:any)=>tag))){
                          return (
                              <>
                                  <Box sx={{gridColumnStart:1,width:'70%'}} className="blog-overview-background">

                                        <Box sx={{gridRow:'1', gap:5}}>
                                              <Stack direction='row' spacing={1}>
                                                  <Avatar sx={{width:26, height:26}} variant='rounded' src={`${authored[post.authors.find((name:any)=>name)].avatar}`}/>
                                                    <TextBlogTitle  textContent={post.slug} variant="subtitle2" shadow2red andali />
                                                    <TextLink serve={Link} to="/users-profile" 
                                                        textContent={post.authors.find((author:any)=>author)} 
                                        ...
                                        <Box sx={{gridColumn:1, gap:1,py:5}}>
                                          <Avatar sx={{
                                                ...
                                                height:310,
                                                  ...
                                                  variant='square'  
                                                    src={`${authored[post.authors.find((author:any)=>author)].img}`}
                                                      alt={`${authored[post.authors.find((author:any)=>author)].name}`}
                                                      srcSet={`${authored[post.authors.find((author:any)=>author)].avatar}?s=${32 * 2} 2x`}
                                                  />
                                        </Box> 

                                        <Box sx={{ gridColumn:1,width:'100%'}} component="div">
                                  ...
                                  <Box sx={{gridColumnStart:smDown ? 1 : 2, width:'30%' }} className="blog-overview-background">
                                      <Stack direction={'row'} spacing={1}>
                                          <Avatar className='square'
                                                sx={{width:50,height:50}} 
                                                variant='square'
                                                  src={`${authored[post.authors.find((author:any)=>author)].avatar}`} 
                                                  />
                                          <Stack direction={'column'}>
                                            {authored[post.authors.find((author:any)=>author)].name}
                                          ...
                                          <MarkdownElement codename={
                                            `<ul class='feature-list'>
                                              <li>
                                                <code>
                                                  ${authored[post.authors.find((author: any) => author)].codename}
                                                </code>
                                              </li>
                                            </ul>`
                                            } />
