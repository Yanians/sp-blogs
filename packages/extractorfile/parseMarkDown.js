...
import textToHash from './textToHash.js';
...
function renderMarkdown(markdown) {
  ...
  if (/[-*+] `([A-Za-z]+)`/g.test(cleaned)) {
    return marked.parse(cleaned, markedOptions);
  }

  return marked.parseInline(cleaned, markedOptions)
    .replace(/(\r?\n){2}/g, '<br>'); // preserve some newlines if needed
}

function getContents(markdown) {
  const context =  marked.parse(markdown,...markedOptions)
...
function removeHeaders(markdown){
  return marked(markdown, ...markedOptions).replace(headerRegExp, '').replace((content) => !emptyRegExp.test(content)); // Remove empty lines
};
